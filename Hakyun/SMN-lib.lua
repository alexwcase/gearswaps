...
function job_post_midcast(spell, action, spellMap, eventArgs)
    ...
    if buffactive['aurorastorm']==1 then
	    equip(sets.midcast.Cure.Aurora)
    end
end
...
function job_get_spell_map(spell)
	...
	elseif enticersRagePacts:contains(spell.english) and spell.english ~= 'Impact' and pet_tp > 350 then
...
function job_self_command(cmdParams, eventArgs)
    ...
    elseif cmdParams[1] == 'buffrotation' then
	    ...
	    if pet.isvalid and pet.name ~= buffavatar then
		    send_command('input /ja "Release" <me>;wait 1;input /ma "'..buffavatar..'" <me>;wait 9;input /ja "'..buffability..'"')
	    elseif not pet.isvalid then
		    send_command('input /ma "'..buffavatar..'" <me>;wait 9;input /ja "'..buffability..'"')
	    else
...
if load_debugging then
	function log_data_structure(data_object)
		...
		local ouf = assert(io.open("D:/Program Files (x86)/Windower4/addons/GearSwap/data/file_log.txt", "a"))
		if ouf then
...
--		event=_global.current_event
--
--		log_data_structure(_global)
--	end
--end
--id = windower.register_event('incoming chunk', update_pet_tp)
--log_data_structure(id)
--info = debug.getinfo(update_pet_tp)
